-- Create database for ADO.NET project
USE master;
GO

IF EXISTS (SELECT name FROM sys.databases WHERE name = 'ORMComparisonAdoNet')
BEGIN
    ALTER DATABASE ORMComparisonAdoNet SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE ORMComparisonAdoNet;
END
GO

CREATE DATABASE ORMComparisonAdoNet;
GO

USE ORMComparisonAdoNet;
GO
